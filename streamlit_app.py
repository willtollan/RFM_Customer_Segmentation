...
with st.expander('Global Surrogate Modelling Benchmarking and Model Selection', expanded=False):
    ...
    with met_col2:
        try:
            df_rf_best_params = load_rf_best_params('data/RF_best_params_updated.csv')
    ...
    with met_col2:
        try:
            df_rf_metrics = load_rf_metrics('data/tuned_RF_key_metrics_updated.csv')
...
try:
    ...
    for class_idx in sorted(LABELS.keys()):
        ...
        class_shap_sums = sweep_shap_output.values[:, :, class_idx].sum(axis=1)
        
        # Calculate raw SHAP f(x) probabilities across all rows for this class
        class_fx_probs = class_base_values + class_shap_sums
        shap_proba_list.append(class_fx_probs)
